<template>
	<view class="content">
		<view class="dateTag">
			<uni-tag size="small" text="今日" @click="changeDate(1)" :type="dateType==1?'warning':'default'" :circle="true"></uni-tag>
			<uni-tag size="small" text="昨日" @click="changeDate(2)" :type="dateType==2?'warning':'default'" :circle="true"></uni-tag>
			<uni-tag size="small" text="本月" @click="changeDate(3)" :type="dateType==3?'warning':'default'" :circle="true"></uni-tag>
			<uni-tag size="small" text="上月" @click="changeDate(4)" :type="dateType==4?'warning':'default'" :circle="true"></uni-tag>
			<uni-tag size="small" text="今年" @click="changeDate(5)" :type="dateType==5?'warning':'default'" :circle="true"></uni-tag>
			<uni-tag size="small" text="累计" @click="changeDate(6)" :type="dateType==6?'warning':'default'" :circle="true"></uni-tag>
		</view>

		<view class="tj-sction">
			<view class="tj-item">
				<text class="num">{{statistic.orderCount}}</text>
				<text>订单数量</text>
			</view>
			<view class="tj-item">
				<text class="num">￥{{statistic.orderAmount}}</text>
				<text>订单金额</text>
			</view>
		</view>
		<view class="title">
			...
			<view class="right">
				<text :class="{'active':topType=='1'}" @click="topType='1'">数量</text>
				<text class="seperator">|</text>
				<text :class="{'active':topType=='2'}" @click="topType='2'">金额</text>
			</view>
		</view>
		<view class="row" v-for="item in statistic.topUnitProductList" v-if="topType=='1'">
			<text class="tit">{{item.productName}}</text>
			<text class="input">x {{item.productUnit}}</text>
		</view>
		<view class="row" v-for="item in statistic.topAmountProductList" v-if="topType=='2'">
			<text class="tit">{{item.productName}}</text>
			<text class="input">￥{{item.productAmount}}</text>
		</view>
		<view class="empty" v-if="topType=='1'&&statistic.topUnitProductList.length === 0">
			<image :src="setSrc" mode="aspectFit"></image>
		</view>
		<view class="empty" v-if="topType=='2'&&statistic.topAmountProductList.length === 0">
			<image :src="setSrc" mode="aspectFit"></image>
		</view>
	</view>
</template>
		...
		methods: {
			...
			changeDate(dateType) {
				...
				if (dateType == 1) { //今天
					...
					startDate = this.$api.dateutil.getFirstDayOfLastXMonth(1);
					endDate = this.$api.dateutil.getLastDayOfLastXMonth(1);
				} else if (dateType == 5) { //今年
					let tDate = this.$api.dateutil.getFirstDayOfThisYear();
					startDate = tDate;
					endDate = currentDate;
				} else if (dateType == 6) { //累计
					let tDate = new Date();
				...
				searchOptions.merchantUuid = this.merchantInfo.merchantUuid;
				//加载中
				this.$api.request.dataStatistic(searchOptions, res => {
					this.statistic = res.body.data;
				}, false);
			},
		}
	}
